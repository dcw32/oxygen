...
def od_chems(d1,d1defs,o2_c,o3_c,sol,nlevs,wbins,h_max):
	o2col=np.zeros([nlevs,wbins])
	o3col=np.zeros([nlevs,wbins])
	for i in range(nlevs):
		for j in range(wbins):
			o2col[i,j]=(i+1)*(1E5*h_max/(nlevs-1))*np.sum(d1[np.where(d1defs=='O2')[0][0],:i+1])
			o3col[i,j]=(i+1)*(1E5*h_max/(nlevs-1))*np.sum(d1[np.where(d1defs=='O3')[0][0],:i+1])
